...
- **链式代理**：在「节点管理」页点击「🔗 链式代理」进入构建模式，按顺序点击任意节点组成一条链（第一个是入口、最后一个是出口）并保存。只在 Clash/mihomo（原生 `relay` 策略组）和 sing-box（`detour` 串联）里生效，通用 V2Ray 分享链接格式不支持链式代理概念，会照常只输出普通节点列表
